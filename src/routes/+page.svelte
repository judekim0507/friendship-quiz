<script lang="ts">
    ...
    let currentStep = $state<Step>(
        data.hasSubmitted ? "already-submitted" : "intro",
    );
    ...
    let progress = $derived(
        ((questionIndex + (currentStep === "quiz" ? 1 : 0)) /
            questions.length) *
            100,
    );
...
<main
    class="min-h-dvh flex flex-col justify-center max-w-2xl mx-auto px-6 py-12 relative"
>
    {#if currentStep === "intro"}
        <section in:fade={{ duration: 400 }} class="flex flex-col gap-8">
            <h1
                class="font-serif text-7xl md:text-8xl leading-[0.85] tracking-tight"
            >
                <span class="italic text-accent">Cosmic</span><br />Twins
            </h1>
            <p class="text-lg md:text-xl text-ink/60 max-w-md">
                A quick friendship quiz to find out who you're most compatible
                with.
            </p>
            <div class="h-24"></div>
            <button
            ...
            <div class="flex flex-col gap-8">
                ...
                <div class="space-y-2">
                    <label
                        ...
                        class="text-xs font-medium tracking-wide text-ink/40 uppercase"
                    >
                        Instagram <span class="normal-case text-ink/30"
                            >(optional)</span
                        >
                    </label>
                    <div
                        class="flex items-center border-b border-ink/15 focus-within:border-ink transition-colors"
                    >
                        <span class="text-lg text-ink/30 select-none mr-1"
                            >@</span
                        >
                        <input
                            id="instagram"
    ...
    {:else if currentStep === "submitted"}
        ...
        >
            <span
                class="border border-ink px-3 py-1 text-[10px] font-black tracking-widest uppercase"
            >
                Submitted
            </span>
            ...
            <p class="text-ink/60 text-lg max-w-sm mt-4">
                We'll calculate your match and send you an email when your
                results are ready.
            </p>

            <div class="w-full h-px bg-ink/10 my-8"></div>

            <p class="text-ink/40 text-sm">Keep an eye on your inbox.</p>
        </section>
    {:else if currentStep === "already-submitted"}
        ...
        >
            <span
                class="border border-ink/40 px-3 py-1 text-[10px] font-black tracking-widest uppercase text-ink/40"
            >
                Already Submitted
            </span>
            ...
            <p class="text-ink/60 text-lg max-w-sm mt-4">
                Only one submission per person allowed. Check your email for
                your results when they're ready.
            </p>
        </section>
    {/if}
</main>
