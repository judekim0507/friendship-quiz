...
const similarityMaps: Record<number, SimilarityMap> = {
  ...
  4: {
    // Colours (spectrum adjacency + warm/cool groupings)
  ...
  5: {
    // Food (regional/flavor profile similarities)
  ...
  9: {
    // What's your vibe? (ambivert bridges both)
    Introvert: { Ambivert: 0.7, Extrovert: 0.4 },
    Extrovert: { Ambivert: 0.7 },
    Ambivert: {},
  },
  10: {
    // When are you most active? (adjacent times overlap)
  ...
  12: {
    ...
    "Sleeping in": {
      "Family time": 0.5,
      "Catching up on work": 0.4,
      "Out with friends": 0.2,
    },
...
export function getOptionSimilarity(
  questionId: number,
  optionA: string,
  optionB: string,
): number {
